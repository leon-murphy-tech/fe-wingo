...
const NavItem = ({ item, level }) => {
  ...
  const isSelected = useMemo(() => {
    if (
      openItem[0] === 'createNew' ||
      openItem[0] === 'editShipment' ||
      openItem[0] === 'createManifest' ||
      openItem[0] === 'editManifest'
    ) {
      return false;
    } else {
      return openItem.findIndex((id) => id === item.id) > -1;
    }
  }, [openItem, item.id]);
  ...
  useEffect(() => {
    ...
    if (pathname.includes('/createNewShipment')) {
      dispatch(activeItem({ openItem: ['createNew'] }));
    }

    if (pathname.includes('/editShipment')) {
      dispatch(activeItem({ openItem: ['editShipment'] }));
    }

    if (pathname.includes('/manifestShipment/create')) {
      dispatch(activeItem({ openItem: ['createManifest'] }));
    }

    if (pathname.includes('/manifestShipment/edit')) {
      dispatch(activeItem({ openItem: ['editManifest'] }));
    }
